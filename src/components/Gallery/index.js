...
class Gallery extends Component {
  state = {currentImg: imagesList[0]}

  selectThumbnail = eachImage => {
    this.setState({currentImg: eachImage})
  }

  render() {
    const {currentImg} = this.state
    const {id, imageUrl, imageAltText} = currentImg
    return (
      <div className="bg-container">
        <img src={imageUrl} alt={imageAltText} className="image" />
        <h1 className="heading">Nature Photography</h1>
        <p className="by">Nature Photography by Rahul</p>
